...
1) Hardlink - как название файла
- меняется файл - меняется hardlink
```
ln file.txt link.txt
```
2) Symlink - ярлык/указатель на другой файл (отдельный файл-ссылка)
- меняется файл - symlink перестает работать (ведет в никуда)
```
